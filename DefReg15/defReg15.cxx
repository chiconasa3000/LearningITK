...
int main(int argc, char * argv[])
{
	...
	if (argc > 10)
	{
		numberOfGridNodesInOneDimensionCoarse = std::stoi(argv[10]);
	}
	...
	if (argc > 11)
	{
		numberOfGridNodesInOneDimensionFine = std::stoi(argv[11]);
	}
	...
	// Software Guide : BeginCodeSnippet
	compositeTransform->RemoveTransform(); // remove bsplineTransformCoarse //Ya no lo considera
	...
	//
	// ************************************************************************
	// Therefore we use the geometric mean of the
	// two numbers (number of TranParams and number of pixels) 
	// to ensure that the number of samples is larger than the number
	// of transform parameters and smaller than the number of samples.
	//
	// Number of Samples  > Transform Parameters     Number of Samples > Number of Pixels 
	//
	// Regulating the number of samples in the Metric is equivalent to performing
	// multi-resolution registration because it is indeed a sub-sampling of the
	// image.
	// **************************************************************************
	const auto numberOfSamples = static_cast<unsigned long>( std::sqrt(static_cast<double>(numberOfBSplineParameters) * static_cast<double>(numberOfPixels)));
	//Geometric Mean between (#params) and (#dePixels)
	//root square de [ (#deParams) * (#dePixels) ]
	...
	std::cout << " Done Ufff you made too much Work!" << std::endl;
	...
	if (argc > 4)
	{
		...
		std::cout << " Writing Difference After!" << std::endl;
	}
	...
	if (argc > 5)
	{
		...
		std::cout << " Writing Difference Before!" << std::endl;
	}
	...
	if (argc > 6)
	{
		...
		std::cout << " Writing Transform Parameters!" << std::endl;
	}
